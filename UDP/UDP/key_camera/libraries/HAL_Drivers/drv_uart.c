...
static rt_err_t ifx_configure(struct rt_serial_device *serial, struct serial_configure *cfg)
{
    ...
    struct ifx_uart *uart = (struct ifx_uart *) serial->parent.user_data;
    RT_ASSERT(uart != RT_NULL);
    RT_ASSERT(uart->config->uart_obj != RT_NULL);

    cy_rslt_t result = CY_RSLT_SUCCESS;
    rt_bool_t newly_initialized = RT_FALSE;

    const cyhal_uart_cfg_t hal_config =
    {
        .data_bits          = 8,
        .stop_bits          = 1,
    ...
    if (!uart->initialized)
    {
        ...
        uart->initialized = RT_TRUE;
        newly_initialized = RT_TRUE;
    }
    ...
    if (result != CY_RSLT_SUCCESS)
    {
        ...
        {
            cyhal_uart_free(uart->config->uart_obj);
            uart->initialized = RT_FALSE;
        }
        return -RT_ERROR;
    }
...
static rt_err_t ifx_control(struct rt_serial_device *serial, int cmd, void *arg)
{
    ...
    case RT_DEVICE_CTRL_CLR_INT:
        uart->config->usart_x->INTR_RX_MASK = 0;
        NVIC_DisableIRQ(uart->config->intrSrc);
        NVIC_ClearPendingIRQ(uart->config->intrSrc);
        break;
...
void rt_hw_uart_init(void)
{
    ...
    for (index = 0; index < obj_num; index++)
    {
        ...
        uart_obj[index].initialized = RT_FALSE;

        uart_obj[index].config->uart_obj = rt_calloc(1, sizeof(cyhal_uart_t));
