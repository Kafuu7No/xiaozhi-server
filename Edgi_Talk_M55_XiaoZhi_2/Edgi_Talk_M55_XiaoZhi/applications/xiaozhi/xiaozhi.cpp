...
#define XIAOZHI_DISCOVERY_TIMEOUT_MS 700
#define XIAOZHI_DISCOVERY_ATTEMPTS 3

static char g_xiaozhi_host[XIAOZHI_HOST_LEN] = XIAOZHI_HOST;
static rt_bool_t g_xiaozhi_discovery_done = RT_FALSE;

/* board1 -> board2 direct UDP meow-water linkage. Config is pushed by cloud
 * over WebSocket as peer_config; trigger counting runs in the meow thread. */
#define MEOW_WATER_MAX_TARGETS 4
#define MEOW_WATER_WINDOW_MS   10000
#define MEOW_WATER_COUNT       3
#define MEOW_WATER_DEBOUNCE_MS 60000

static char      g_b2_ips[MEOW_WATER_MAX_TARGETS][16] = {0};
static int       g_b2_ip_count = 0;
static int       g_b2_port = 8848;
static int       g_auto_on_meow = 0;
static int       g_water_seconds = 5;
static rt_tick_t g_meow_window[MEOW_WATER_COUNT] = {0};
static int       g_meow_window_len = 0;
static rt_tick_t g_meow_last_fire = 0;
static int       g_meow_has_fired = 0;
...
const char *xz_get_host(void)
{
    return g_xiaozhi_host;
}

static void xz_set_host(const char *host)
{
    if (!host || host[0] == '\0')
    {
        return;
    ...
    rt_strncpy(g_xiaozhi_host, host, sizeof(g_xiaozhi_host) - 1);
    g_xiaozhi_host[sizeof(g_xiaozhi_host) - 1] = '\0';
}

static rt_bool_t xz_parse_discovery_response(const char *payload, char *host, rt_size_t host_len)
{
    const char *host_key;
    const char *host_start;
    const char *host_end;

    if (!payload || !host || host_len == 0)
    {
        return RT_FALSE;
    }

    if (rt_strncmp(payload, XIAOZHI_DISCOVERY_RESPONSE, rt_strlen(XIAOZHI_DISCOVERY_RESPONSE)) != 0)
    {
        return RT_FALSE;
    }

    host_key = strstr(payload, "host=");
    if (host_key)
    {
        host_start = host_key + 5;
        host_end = host_start;
        while (*host_end && *host_end != ' ' && *host_end != '\r' && *host_end != '\n')
        {
            host_end++;
        }
    }
    ...
    {
        host_start = payload + rt_strlen(XIAOZHI_DISCOVERY_RESPONSE);
        while (*host_start == ' ')
        {
            host_start++;
        }
        if (strchr(host_start, '=') != RT_NULL)
        {
            return RT_FALSE;
        }
        host_end = host_start;
    ...
    rt_size_t len = host_end - host_start;
    if (len == 0 || len >= host_len)
    {
        return RT_FALSE;
    }

    rt_memcpy(host, host_start, len);
    host[len] = '\0';
    return RT_TRUE;
}

static rt_err_t xz_discover_cloud_once(void)
{
    int sock = -1;
    int opt = 1;
    ...
    char buffer[128] = {0};
    char discovered_host[XIAOZHI_HOST_LEN] = {0};
    int received;
    ...
    if (sock < 0)
    {
        LOG_W("Discovery socket create failed");
        return -RT_ERROR;
    }

    lwip_setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &opt, sizeof(opt));
    timeout.tv_sec = 0;
    timeout.tv_usec = XIAOZHI_DISCOVERY_TIMEOUT_MS * 1000;
    lwip_setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, &timeout, sizeof(timeout));

    rt_memset(&dest, 0, sizeof(dest));
    dest.sin_family = AF_INET;
    dest.sin_port = htons(XIAOZHI_DISCOVERY_PORT);

    /*
     * lwIP refuses to route 255.255.255.255 when no default route covers it
     * (which is the usual case on a freshly DHCP'd Wi-Fi link), so fan out
     * to multiple destinations: subnet-directed broadcast, default gateway
     * (which doubles as a direct hit when the cloud server runs on the AP
     * itself, e.g. a Windows mobile hotspot), and finally the limited
     * broadcast as a last resort. Any one being delivered is enough.
     */
    uint32_t dest_ips[3];
    int dest_cnt = 0;
    struct netdev *nd = netdev_default;
    if (nd != RT_NULL)
    {
        uint32_t ip_be = ip4_addr_get_u32(ip_2_ip4(&nd->ip_addr));
        uint32_t nm_be = ip4_addr_get_u32(ip_2_ip4(&nd->netmask));
        uint32_t gw_be = ip4_addr_get_u32(ip_2_ip4(&nd->gw));
        if (ip_be != 0 && nm_be != 0)
        {
            dest_ips[dest_cnt++] = (ip_be & nm_be) | ~nm_be;
        }
        if (gw_be != 0)
        {
            dest_ips[dest_cnt++] = gw_be;
        }
    }
    dest_ips[dest_cnt++] = htonl(INADDR_BROADCAST);

    int sent_ok = 0;
    for (int i = 0; i < dest_cnt; i++)
    {
        dest.sin_addr.s_addr = dest_ips[i];
        if (lwip_sendto(sock, XIAOZHI_DISCOVERY_REQUEST, rt_strlen(XIAOZHI_DISCOVERY_REQUEST), 0,
    ...
    if (sent_ok == 0)
    {
        LOG_W("Discovery broadcast failed (all destinations)");
        goto exit;
    }

    received = lwip_recvfrom(sock, buffer, sizeof(buffer) - 1, 0,
    ...
    buffer[received] = '\0';
    if (xz_parse_discovery_response(buffer, discovered_host, sizeof(discovered_host)))
    {
        xz_set_host(discovered_host);
    }
    else
    {
        xz_set_host(inet_ntoa(sender.sin_addr));
    }

    g_xiaozhi_discovery_done = RT_TRUE;
    LOG_I("Discovered XiaoZhi cloud: %s:%d", xz_get_host(), XIAOZHI_WS_PORT);
    ret = RT_EOK;
...
/* Send board2 one pump-start packet + little-endian duration_ms.
 * with_photo=RT_TRUE  -> 0x12: 继电器吸合并拍照（仅“3 次猫叫触发”使用）
 * with_photo=RT_FALSE -> 0x10: 仅继电器吸合，不拍照（手动/定时/WS/shell 使用） */
static int meow_water_send(const char *ip, int port, uint32_t duration_ms,
                           rt_bool_t with_photo)
{
    if (!ip || ip[0] == '\0')
        return 0;
    ...
    if (sock < 0)
    {
        LOG_W("meow_water: socket failed for %s:%d", ip, port);
        return 0;
    }
    ...
    {
        lwip_close(sock);
        LOG_W("meow_water: invalid target ip '%s'", ip);
        return 0;
    }

    uint8_t pkt[5];
    pkt[0] = with_photo ? 0x12 : 0x10;
    pkt[1] = (uint8_t)(duration_ms & 0xFF);
    pkt[2] = (uint8_t)((duration_ms >> 8) & 0xFF);
    pkt[3] = (uint8_t)((duration_ms >> 16) & 0xFF);
    pkt[4] = (uint8_t)((duration_ms >> 24) & 0xFF);

    int sent = lwip_sendto(sock, pkt, sizeof(pkt), 0,
                           (struct sockaddr *)&dest, sizeof(dest));
    lwip_close(sock);
    if (sent != (int)sizeof(pkt))
    {
        LOG_W("meow_water: send failed to %s:%d (%d/%d)",
              ip, port, sent, (int)sizeof(pkt));
        return 0;
    }

    LOG_I("meow_water: sent %u ms to %s:%d", (unsigned)duration_ms, ip, port);
    return 1;
}

static void meow_water_snapshot(int *auto_on, int *port, int *delay_s,
                                int *n, char ips[MEOW_WATER_MAX_TARGETS][16])
{
    rt_enter_critical();
    *auto_on = g_auto_on_meow;
    *port = g_b2_port;
    *delay_s = g_water_seconds;
    *n = g_b2_ip_count;
    rt_memcpy(ips, g_b2_ips, sizeof(g_b2_ips));
    rt_exit_critical();
}

static int meow_water_fire_targets(char ips[MEOW_WATER_MAX_TARGETS][16],
                                   int n, int port, uint32_t duration_ms,
                                   const char *reason, rt_bool_t with_photo)
{
    if (duration_ms > 600000u)
        duration_ms = 600000u;

    if (n <= 0)
    {
        LOG_W("meow_water: no board2 targets for %s", reason ? reason : "fire");
        return 0;
    }

    int sent = 0;
    for (int i = 0; i < n; i++)
        sent += meow_water_send(ips[i], port, duration_ms, with_photo);

    LOG_I("meow_water: fired %u ms to %d/%d target(s), reason=%s",
          (unsigned)duration_ms, sent, n, reason ? reason : "unknown");
    return sent;
}

extern "C" void meow_water_on_meow(void)
{
    rt_tick_t now = rt_tick_get();
    rt_tick_t window_ticks = rt_tick_from_millisecond(MEOW_WATER_WINDOW_MS);
    rt_tick_t debounce_ticks = rt_tick_from_millisecond(MEOW_WATER_DEBOUNCE_MS);

    int auto_on;
    int port;
    int delay_s;
    int n;
    char ips[MEOW_WATER_MAX_TARGETS][16];

    meow_water_snapshot(&auto_on, &port, &delay_s, &n, ips);

    if (!auto_on || n <= 0)
    {
        LOG_I("meow_water: ignored meow auto=%d targets=%d", auto_on, n);
        g_meow_window_len = 0;
        return;
    }

    if (g_meow_window_len < MEOW_WATER_COUNT)
    {
        g_meow_window[g_meow_window_len++] = now;
    }
    else
    {
        for (int i = 1; i < MEOW_WATER_COUNT; i++)
            g_meow_window[i - 1] = g_meow_window[i];
        g_meow_window[MEOW_WATER_COUNT - 1] = now;
    }

    int within = 0;
    for (int i = 0; i < g_meow_window_len; i++)
    {
        if ((now - g_meow_window[i]) <= window_ticks)
            within++;
    }
    LOG_I("meow_water: count=%d/%d targets=%d delay=%d",
          within, MEOW_WATER_COUNT, n, delay_s);
    if (within < MEOW_WATER_COUNT)
        return;

    if (g_meow_has_fired && (now - g_meow_last_fire) < debounce_ticks)
    {
        LOG_I("meow_water: debounce active");
        return;
    }

    uint32_t duration_ms = (uint32_t)delay_s * 1000u;
    /* “3 次猫叫”触发：发 0x12 让放水板放水并同时拍照（手动/定时放水不拍照）。 */
    meow_water_fire_targets(ips, n, port, duration_ms, "meow", RT_TRUE);

    /* 通知云端记录这次“猫叫触发”的出水（板子直连放水板，云端否则不知道）。
     * 云端据此在“饮水管理-出水记录”里写一条：触发方式=猫叫触发，时长=delay_s 秒。 */
    {
        const char *sid = xz_ws_get_session_id();
        if (sid && sid[0] != '\0')
        {
            char msg[128];
            rt_snprintf(msg, sizeof(msg),
                        "{\"session_id\":\"%s\",\"type\":\"meow_water\","
                        "\"duration\":%d,\"ts\":%u}",
                        sid, delay_s, (unsigned)rt_tick_get());
            (void)ws_send_text_locked(msg);
        }
    }

    g_meow_last_fire = now;
    g_meow_has_fired = 1;
    g_meow_window_len = 0;
}

static rt_err_t xz_discover_cloud(rt_bool_t force)
{
    if (g_xiaozhi_discovery_done && !force)
    {
        return RT_EOK;
    }
    ...
    for (int i = 0; i < XIAOZHI_DISCOVERY_ATTEMPTS; i++)
    {
        if (xz_discover_cloud_once() == RT_EOK)
        {
            return RT_EOK;
        }
    ...
    LOG_W("XiaoZhi cloud discovery failed, using configured host %s", xz_get_host());
    return -RT_ERROR;
}
        ...
        if (evt & BUTTON_EVENT_PRESSED)
        {
            ...
            else
            {
                ...
                if (g_app.state != kDeviceStateListening)
                {
                    ...
                    xiaozhi_ui_chat_status("   聆听中");
                    xiaozhi_ui_chat_output("聆听中...");
                    xiaozhi_ui_set_emoji("surprised");
...
{
    if (!msg || !g_app.ws.is_connected || !g_app.ws.ws_write_mutex)
    {
        return RT_FALSE;
    }
    ...
    if (rt_mutex_take(g_app.ws.ws_write_mutex, RT_WAITING_NO) != RT_EOK)
    {
        LOG_D("WebSocket write busy, skip text frame\n");
        return RT_FALSE;
    }
    ...
    if (g_app.ws.is_connected)
    {
        err_t result = wsock_write(&g_app.ws.clnt, msg, strlen(msg), OPCODE_TEXT);
        if (result == ERR_OK)
        {
            ok = RT_TRUE;
        }
        ...
        {
            LOG_E("Failed to send websocket text: %d\n", result);
            if (result == ERR_CLSD || result == ERR_RST)
            {
                g_app.ws.is_connected = 0;
            }
...
void reconnect_websocket(void)
{
    ...
    while (retry-- > 0)
    {
        ...
        wsock_init(&g_app.ws.clnt, XIAOZHI_WS_SSL_ENABLED, 1, my_wsapp_fn);
        result = wsock_connect(&g_app.ws.clnt, MAX_WSOCK_HDR_LEN,
                               xz_get_host(), XIAOZHI_WSPATH,
                               XIAOZHI_WS_PORT, XIAOZHI_TOKEN, NULL,
                               "Protocol-Version: 1\r\nDevice-Id: %s\r\nClient-Id: %s\r\n",
...
static void send_meow_status(rt_bool_t enabled, int result, const char *message)
{
    ...
    rt_snprintf(status_msg, sizeof(status_msg),
                "{\"session_id\":\"%s\",\"type\":\"meow_status\","
                "\"enabled\":%s,\"result\":%d,\"message\":\"%s\",\"ts\":%u}",
                session_id,
                enabled ? "true" : "false",
                result,
                message ? message : "",
                (unsigned)rt_tick_get());
    (void)ws_send_text_locked(status_msg);
}

/* Public wrapper so the device UI (xiaozhi_ui.c) can report meow on/off to the
 * cloud when the user toggles it on the screen (board1 -> cloud sync). */
extern "C" void xz_notify_meow_state(rt_bool_t enabled)
{
    send_meow_status(enabled, 0, enabled ? "running" : "stopped");
}
...
void Message_handle(const uint8_t *data, uint16_t len)
{
    ...
    else if (strcmp(type, "stt") == 0)
    {
        ...
        if (text && text[0] != '\0')
        {
            xiaozhi_ui_chat_output((char *)text);
        }

        /* 本地关键字：通过语音打开猫叫检测系统
         * 例如：“小智，请打开猫叫检测系统”、“打开猫叫检测”等。 */
        if (text && strstr(text, "打开猫叫检测") != RT_NULL)
        {
            LOG_I("stt command: open meow detector via voice");
            xiaozhi_ui_enter_meow_mode_from_voice();
        }
    }
    else if (strcmp(type, "meow_control") == 0)
    {
        char *action = my_json_string(root, "action");
        if (strcmp(action, "start") == 0)
        {
            int ret;

            LOG_I("meow_control: start");
            if (xz_wakeword_is_enabled())
            {
                LOG_I("meow_control: stopping wakeword before meow detector");
                xz_wakeword_stop();
            }
            if (xz_mic_is_enabled())
            {
            ...
            ret = meow_detect_start();
            if (ret == RT_EOK)
            {
                send_meow_status(RT_TRUE, ret, "running");
                xiaozhi_ui_show_meow_screen();   /* 云端启动检测 -> 板子切到猫叫页 */
            }
            else
            {
                send_meow_status(RT_FALSE, ret, "start_failed");
                if (g_app.state == kDeviceStateIdle && !xz_wakeword_is_enabled())
                {
                    xz_wakeword_start();
                }
            }
        ...
        else if (strcmp(action, "stop") == 0)
        {
            ...
            LOG_I("meow_control: stop");
            ret = meow_detect_stop();
            send_meow_status(RT_FALSE, ret, "stopped");
            xiaozhi_ui_show_main_screen();   /* 云端停止检测 -> 板子回主页 */
            if (g_app.state == kDeviceStateIdle && !xz_wakeword_is_enabled())
            {
                xz_wakeword_start();
            }
        ...
        {
            LOG_W("Unknown meow_control action: %s", action);
            send_meow_status(RT_FALSE, -RT_EINVAL, "unknown_action");
        }
    }
    ...
    else if (strcmp(type, "peer_config") == 0)
    {
        cJSON *ips = cJSON_GetObjectItem(root, "board2_ips");
        char target_ips[MEOW_WATER_MAX_TARGETS][16] = {0};
        int n = 0;

        if (ips && cJSON_IsArray(ips))
        {
            int total = cJSON_GetArraySize(ips);
            for (int i = 0; i < total && n < MEOW_WATER_MAX_TARGETS; i++)
            {
                cJSON *item = cJSON_GetArrayItem(ips, i);
                if (item && cJSON_IsString(item) && item->valuestring)
                {
                    strncpy(target_ips[n], item->valuestring, sizeof(target_ips[n]) - 1);
                    target_ips[n][sizeof(target_ips[n]) - 1] = '\0';
                    n++;
                }
            }
        }
        ...
        cJSON *auto_item = cJSON_GetObjectItem(root, "autoOnMeow");
        cJSON *delay_item = cJSON_GetObjectItem(root, "delaySeconds");
        int port = (port_item && cJSON_IsNumber(port_item)) ? port_item->valueint : 8848;
        int auto_on = (auto_item && cJSON_IsBool(auto_item)) ? (cJSON_IsTrue(auto_item) ? 1 : 0) : 0;
        int delay_s = (delay_item && cJSON_IsNumber(delay_item)) ? delay_item->valueint : 15;

        rt_enter_critical();
        g_b2_ip_count = n;
        rt_memset(g_b2_ips, 0, sizeof(g_b2_ips));
        for (int i = 0; i < n; i++)
        {
            rt_memcpy(g_b2_ips[i], target_ips[i], sizeof(g_b2_ips[i]));
            g_b2_ips[i][sizeof(g_b2_ips[i]) - 1] = '\0';
        }
        g_b2_port = (port > 0) ? port : 8848;
        g_auto_on_meow = auto_on;
        g_water_seconds = (delay_s > 0) ? delay_s : 15;
        rt_exit_critical();

        LOG_I("peer_config: %d board2 ip(s), auto=%d, delay=%d",
              n, auto_on, g_water_seconds);
    }
    else if (strcmp(type, "meow_water_fire") == 0)
    {
        int auto_on;
        int port;
        int delay_s;
        ...
        int seconds = (duration_item && cJSON_IsNumber(duration_item))
                          ? duration_item->valueint
                          : delay_s;
        if (seconds <= 0)
            seconds = 15;

        LOG_I("meow_water_fire: ws request seconds=%d auto=%d targets=%d port=%d",
              seconds, auto_on, n, port);
        meow_water_fire_targets(ips, n, port, (uint32_t)seconds * 1000u, "ws", RT_FALSE);
    }
...
{
    const char *hostname = xz_get_host();
    ip_addr_t addr = {0};
...
char *get_xiaozhi_ws(void)
{
    ...
    rt_snprintf(xiaozhi_url, GET_URL_LEN_MAX, GET_URI, xz_get_host());
...
static int xz_host_cmd(int argc, char **argv)
{
    if (argc < 2)
    {
        rt_kprintf("Current XiaoZhi host: %s:%d\n", xz_get_host(), XIAOZHI_WS_PORT);
        rt_kprintf("Usage: xz_host <server-ip-or-host>\n");
        return RT_EOK;
    }

    xz_set_host(argv[1]);
    g_xiaozhi_discovery_done = RT_TRUE;
    rt_kprintf("XiaoZhi host set to %s:%d\n", xz_get_host(), XIAOZHI_WS_PORT);
    return RT_EOK;
}
MSH_CMD_EXPORT_ALIAS(xz_host_cmd, xz_host, Show or set XiaoZhi cloud host);

static int xz_discover_cmd(int argc, char **argv)
{
    rt_err_t ret = xz_discover_cloud(RT_TRUE);
    if (ret == RT_EOK)
    {
        rt_kprintf("Discovered XiaoZhi cloud: %s:%d\n", xz_get_host(), XIAOZHI_WS_PORT);
    }
    else
    {
        rt_kprintf("Discovery failed, current host: %s:%d\n", xz_get_host(), XIAOZHI_WS_PORT);
    }
    return ret;
}
MSH_CMD_EXPORT_ALIAS(xz_discover_cmd, xz_discover, Discover XiaoZhi cloud host on LAN);

static int meow_water_status_cmd(int argc, char **argv)
{
    (void)argc;
    (void)argv;
    ...
    rt_kprintf("meow_water: auto=%d delay=%d port=%d targets=%d window=%d fired=%d\n",
               auto_on, delay_s, port, n, g_meow_window_len, g_meow_has_fired);
    for (int i = 0; i < n; i++)
        rt_kprintf("  target[%d]=%s\n", i, ips[i]);
    return RT_EOK;
}
MSH_CMD_EXPORT_ALIAS(meow_water_status_cmd, meow_water_status, Show meow-water peer config);

static int meow_water_fire_cmd(int argc, char **argv)
{
    int auto_on;
    int port;
    int delay_s;
    ...
    rt_kprintf("meow_water_fire: auto=%d seconds=%d targets=%d port=%d\n",
               auto_on, seconds, n, port);
    int sent = meow_water_fire_targets(ips, n, port, (uint32_t)seconds * 1000u, "shell", RT_FALSE);
    rt_kprintf("meow_water_fire: sent=%d/%d\n", sent, n);
    return sent > 0 ? RT_EOK : -RT_ERROR;
}
MSH_CMD_EXPORT_ALIAS(meow_water_fire_cmd, meow_water_fire, Fire board2 pump from board1);
