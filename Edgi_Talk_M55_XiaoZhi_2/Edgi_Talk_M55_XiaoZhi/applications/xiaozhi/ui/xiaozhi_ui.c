...
#define ENV_CHART_POINTS    30

#ifndef XIAOZHI_UI_USE_3D_EMOJI
#define XIAOZHI_UI_USE_3D_EMOJI 0
#endif
...
static lv_obj_t *s_emoji_fallback_canvas;
static lv_obj_t *s_main_container;
static lv_obj_t *s_header_row;
static lv_obj_t *s_img_container;
static lv_obj_t *s_face_left_eye = RT_NULL;
static lv_obj_t *s_face_right_eye = RT_NULL;
static lv_obj_t *s_face_mouth = RT_NULL;
static lv_obj_t *s_face_label = RT_NULL;
...
/* 2D fallback canvas buffer (big emoji area) */
static uint8_t s_emoji_2d_buf[420 * 280 * 4] __attribute__((aligned(64))) rt_section(".m33_m55_shared_hyperram");
...
static lv_obj_t *s_btn_env = RT_NULL;
static lv_obj_t *s_lbl_env = RT_NULL;
static lv_obj_t *s_meow_cat_canvas = RT_NULL;
static lv_obj_t *s_env_screen = RT_NULL;
static lv_obj_t *s_env_panel = RT_NULL;
static lv_obj_t *s_env_value_temp = RT_NULL;
static lv_obj_t *s_env_value_humi = RT_NULL;
static lv_obj_t *s_env_unit_temp = RT_NULL;
static lv_obj_t *s_env_unit_humi = RT_NULL;
static lv_obj_t *s_env_hint = RT_NULL;
static lv_obj_t *s_env_chart = RT_NULL;
static lv_chart_series_t *s_env_ser_temp = RT_NULL;
static lv_chart_series_t *s_env_ser_humi = RT_NULL;
static int32_t s_env_temp_points[ENV_CHART_POINTS];
static int32_t s_env_humi_points[ENV_CHART_POINTS];
static rt_bool_t s_env_mode = RT_FALSE;
static rt_bool_t s_env_valid = RT_FALSE;
static float s_env_temp = 0.0f;
static float s_env_humi = 0.0f;

static void env_btn_event_cb(lv_event_t *e);
static void env_back_btn_event_cb(lv_event_t *e);
static void env_build_screen(void);
static void env_refresh_labels(void);
...
static inline void emoji_canvas_px(lv_obj_t *canvas, int w, int h, int x, int y, lv_color_t col)
{
    if (!canvas)
        return;
    if (x < 0 || y < 0 || x >= w || y >= h)
...
static void emoji_canvas_disc(lv_obj_t *canvas, int w, int h, int cx, int cy, int r, lv_color_t col)
{
    int rr = r * r;
    for (int y = cy - r; y <= cy + r; y++)
    {
        for (int x = cx - r; x <= cx + r; x++)
        {
            ...
            int dy = y - cy;
            if ((dx * dx + dy * dy) <= rr)
                emoji_canvas_px(canvas, w, h, x, y, col);
        }
    }
}

static void emoji_canvas_hline(lv_obj_t *canvas, int w, int h, int x0, int x1, int y, int thick, lv_color_t col)
{
    if (x0 > x1)
    {
        int t = x0;
    ...
    for (int yy = y - thick / 2; yy <= y + thick / 2; yy++)
    {
        for (int x = x0; x <= x1; x++)
            emoji_canvas_px(canvas, w, h, x, yy, col);
    }
}

static void emoji_draw_2d_fallback(lv_obj_t *canvas, int index)
{
    if (!canvas)
        return;
    ...
    const int W = 420;
    const int H = 280;
    lv_canvas_set_buffer(canvas, s_emoji_2d_buf, W, H, LV_COLOR_FORMAT_ARGB8888);
    lv_canvas_fill_bg(canvas, lv_color_black(), LV_OPA_TRANSP);

    if (index < 0 || index >= EMOJI_NUM)
        index = 0;

    const lv_color_t face = lv_color_hex(0xFFD95A);
    const lv_color_t eye = lv_color_hex(0x1A1A1A);
    const lv_color_t mouth = lv_color_hex(0x1A1A1A);
    const lv_color_t blush = lv_color_hex(0xFF8FB1);
    ...
    emoji_canvas_disc(canvas, W, H, cx, cy, fr, face);

    switch (index)
    {
    case 1:
    case 2:
        emoji_canvas_hline(canvas, W, H, cx - ex - 10, cx - ex + 10, cy - ey, 4, eye);
        emoji_canvas_hline(canvas, W, H, cx + ex - 10, cx + ex + 10, cy - ey, 4, eye);
        emoji_canvas_hline(canvas, W, H, cx - 34, cx + 34, cy + 34, 6, mouth);
        break;
    case 4:
    case 6:
        emoji_canvas_disc(canvas, W, H, cx - ex, cy - ey, 7, eye);
        emoji_canvas_disc(canvas, W, H, cx + ex, cy - ey, 7, eye);
        emoji_canvas_hline(canvas, W, H, cx - 28, cx + 28, cy + 40, 4, mouth);
        emoji_canvas_disc(canvas, W, H, cx - ex - 8, cy - ey + 24, 6, lv_color_hex(0x66C2FF));
        emoji_canvas_disc(canvas, W, H, cx + ex + 8, cy - ey + 24, 6, lv_color_hex(0x66C2FF));
        break;
    case 5:
        emoji_canvas_hline(canvas, W, H, cx - ex - 16, cx - ex + 8, cy - ey - 8, 4, eye);
        emoji_canvas_hline(canvas, W, H, cx + ex - 8, cx + ex + 16, cy - ey - 8, 4, eye);
        emoji_canvas_disc(canvas, W, H, cx - ex, cy - ey + 8, 6, eye);
        emoji_canvas_disc(canvas, W, H, cx + ex, cy - ey + 8, 6, eye);
        emoji_canvas_hline(canvas, W, H, cx - 20, cx + 20, cy + 36, 5, mouth);
        break;
    case 8:
        emoji_canvas_hline(canvas, W, H, cx - ex - 10, cx - ex + 10, cy - ey + 4, 3, eye);
        emoji_canvas_hline(canvas, W, H, cx + ex - 10, cx + ex + 10, cy - ey + 4, 3, eye);
        emoji_canvas_hline(canvas, W, H, cx - 18, cx + 18, cy + 36, 3, mouth);
        break;
    case 9:
    case 10:
        emoji_canvas_disc(canvas, W, H, cx - ex, cy - ey, 8, eye);
        emoji_canvas_disc(canvas, W, H, cx + ex, cy - ey, 8, eye);
        emoji_canvas_disc(canvas, W, H, cx, cy + 34, 12, mouth);
        break;
    case 12:
        emoji_canvas_hline(canvas, W, H, cx - ex - 8, cx - ex + 8, cy - ey, 3, eye);
        emoji_canvas_disc(canvas, W, H, cx + ex, cy - ey, 7, eye);
        emoji_canvas_hline(canvas, W, H, cx - 20, cx + 28, cy + 34, 4, mouth);
        break;
    default:
        emoji_canvas_disc(canvas, W, H, cx - ex, cy - ey, 7, eye);
        emoji_canvas_disc(canvas, W, H, cx + ex, cy - ey, 7, eye);
        emoji_canvas_hline(canvas, W, H, cx - 24, cx + 24, cy + 34, 4, mouth);
        break;
    }

    emoji_canvas_disc(canvas, W, H, cx - 56, cy + 12, 9, blush);
    emoji_canvas_disc(canvas, W, H, cx + 56, cy + 12, 9, blush);
}
...
static void meow_btn_event_cb(lv_event_t *e)
{
    ...
    if (meow_detect_start() == RT_EOK)
    {
        ...
        xz_notify_meow_state(RT_TRUE);   /* 板子进入猫叫页 -> 通知云端启动 */
    }
    else
    {
...
static void env_refresh_labels(void)
{
    ...
    if (s_env_valid)
    {
        char tbuf[24];
        char hbuf[24];
        rt_snprintf(tbuf, sizeof(tbuf), "%.1f", s_env_temp);
        rt_snprintf(hbuf, sizeof(hbuf), "%.1f", s_env_humi);

        if (s_env_value_temp)
            lv_label_set_text(s_env_value_temp, tbuf);
        if (s_env_value_humi)
            lv_label_set_text(s_env_value_humi, hbuf);
        if (s_env_unit_temp)
            lv_label_set_text(s_env_unit_temp, "C");
        if (s_env_unit_humi)
            lv_label_set_text(s_env_unit_humi, "%");
        if (s_env_hint)
            lv_label_set_text(s_env_hint, "Updated");

        if (s_env_chart && s_env_ser_temp && s_env_ser_humi)
        {
            for (int i = 0; i < ENV_CHART_POINTS - 1; i++)
            {
                s_env_temp_points[i] = s_env_temp_points[i + 1];
                s_env_humi_points[i] = s_env_humi_points[i + 1];
            }
            s_env_temp_points[ENV_CHART_POINTS - 1] = (int32_t)(s_env_temp * 10.0f);
            s_env_humi_points[ENV_CHART_POINTS - 1] = (int32_t)(s_env_humi * 10.0f);

            for (int i = 0; i < ENV_CHART_POINTS; i++)
            {
                lv_chart_set_value_by_id(s_env_chart, s_env_ser_temp, i, s_env_temp_points[i]);
                lv_chart_set_value_by_id(s_env_chart, s_env_ser_humi, i, s_env_humi_points[i]);
            }
            lv_chart_refresh(s_env_chart);
        }
    }
    else
    {
        if (s_env_value_temp)
            lv_label_set_text(s_env_value_temp, "--.-");
        if (s_env_value_humi)
            lv_label_set_text(s_env_value_humi, "--.-");
        if (s_env_unit_temp)
            lv_label_set_text(s_env_unit_temp, "C");
        if (s_env_unit_humi)
            lv_label_set_text(s_env_unit_humi, "%");
        if (s_env_hint)
            lv_label_set_text(s_env_hint, "Waiting for sensor data...");
    }
}

/**
 * @brief Temp/Humi button event callback
 */
static void env_btn_event_cb(lv_event_t *e)
{
    lv_event_code_t code = lv_event_get_code(e);
    ...
    if (s_meow_mode)
    {
        ...
        xz_notify_meow_state(RT_FALSE);   /* 从猫叫页切到温湿度页 -> 通知云端停止 */
    }

    float temp_c = 0.0f;
    float humi_rh = 0.0f;
    if (env_read_once(&temp_c, &humi_rh) == RT_EOK)
    {
        s_env_temp = temp_c;
        s_env_humi = humi_rh;
        s_env_valid = RT_TRUE;
...
static void env_build_screen(void)
{
    s_env_screen = lv_obj_create(NULL);
    lv_obj_clear_flag(s_env_screen, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_style_bg_color(s_env_screen, lv_color_hex(0x0A0D12), LV_PART_MAIN | LV_STATE_DEFAULT);

    s_env_panel = lv_obj_create(s_env_screen);
    lv_obj_remove_flag(s_env_panel, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_size(s_env_panel, LV_PCT(90), LV_PCT(86));
    lv_obj_center(s_env_panel);
    lv_obj_set_style_bg_color(s_env_panel, lv_color_hex(0x101722), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(s_env_panel, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(s_env_panel, SCALE_DPX(18), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(s_env_panel, 2, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(s_env_panel, lv_color_hex(0x2B3A55), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(s_env_panel, SCALE_DPX(18), LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_t *title = lv_label_create(s_env_panel);
    ...
    lv_obj_t *subtitle = lv_label_create(s_env_panel);
    lv_label_set_text(subtitle, "AHT20 - i2c2");
    lv_obj_add_style(subtitle, &s_style_20, 0);
    lv_obj_set_width(subtitle, LV_PCT(100));
    lv_obj_set_style_text_align(subtitle, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(subtitle, lv_color_hex(0x93A4BD), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align_to(subtitle, title, LV_ALIGN_OUT_BOTTOM_MID, 0, SCALE_DPX(-2));

    lv_obj_t *btn_back = lv_button_create(s_env_panel);
    lv_obj_set_size(btn_back, SCALE_DPX(180), SCALE_DPX(54));
    lv_obj_align(btn_back, LV_ALIGN_BOTTOM_MID, 0, 0);
    lv_obj_add_event_cb(btn_back, env_back_btn_event_cb, LV_EVENT_ALL, NULL);
    ...
    lv_obj_t *card_row = lv_obj_create(s_env_panel);
    lv_obj_remove_flag(card_row, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_style_bg_opa(card_row, LV_OPA_0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(card_row, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(card_row, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_size(card_row, LV_PCT(100), SCALE_DPX(160));
    lv_obj_align_to(card_row, subtitle, LV_ALIGN_OUT_BOTTOM_MID, 0, SCALE_DPX(14));
    lv_obj_set_flex_flow(card_row, LV_FLEX_FLOW_ROW);
    lv_obj_set_flex_align(card_row, LV_FLEX_ALIGN_SPACE_BETWEEN, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);

    lv_obj_t *card_temp = lv_obj_create(card_row);
    lv_obj_remove_flag(card_temp, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_clear_flag(card_temp, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_set_size(card_temp, LV_PCT(48), SCALE_DPX(150));
    lv_obj_set_style_radius(card_temp, SCALE_DPX(18), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(card_temp, lv_color_hex(0x0E1622), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(card_temp, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(card_temp, 1, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(card_temp, lv_color_hex(0x2B3A55), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(card_temp, SCALE_DPX(14), LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_t *temp_tag = lv_label_create(card_temp);
    lv_label_set_text(temp_tag, "Temperature");
    lv_obj_add_style(temp_tag, &s_style_20, 0);
    lv_obj_set_style_text_color(temp_tag, lv_color_hex(0xFFE0B2), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(temp_tag, LV_ALIGN_TOP_LEFT, 0, 0);

    s_env_value_temp = lv_label_create(card_temp);
    lv_label_set_text(s_env_value_temp, "--.-");
    lv_obj_add_style(s_env_value_temp, &s_style_30, 0);
    lv_obj_set_style_text_color(s_env_value_temp, lv_color_hex(0xFFB74D), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(s_env_value_temp, LV_ALIGN_LEFT_MID, 0, SCALE_DPX(14));

    s_env_unit_temp = lv_label_create(card_temp);
    lv_label_set_text(s_env_unit_temp, "C");
    lv_obj_add_style(s_env_unit_temp, &s_style_20, 0);
    lv_obj_set_style_text_color(s_env_unit_temp, lv_color_hex(0xFFCC80), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align_to(s_env_unit_temp, s_env_value_temp, LV_ALIGN_OUT_RIGHT_MID, SCALE_DPX(40), 0);

    lv_obj_t *card_humi = lv_obj_create(card_row);
    lv_obj_remove_flag(card_humi, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_clear_flag(card_humi, LV_OBJ_FLAG_CLICKABLE);
    lv_obj_set_size(card_humi, LV_PCT(48), SCALE_DPX(150));
    lv_obj_set_style_radius(card_humi, SCALE_DPX(18), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(card_humi, lv_color_hex(0x0E1622), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_opa(card_humi, LV_OPA_COVER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(card_humi, 1, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(card_humi, lv_color_hex(0x2B3A55), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(card_humi, SCALE_DPX(14), LV_PART_MAIN | LV_STATE_DEFAULT);

    lv_obj_t *humi_tag = lv_label_create(card_humi);
    lv_label_set_text(humi_tag, "Humidity");
    lv_obj_add_style(humi_tag, &s_style_20, 0);
    lv_obj_set_style_text_color(humi_tag, lv_color_hex(0xB3E5FC), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(humi_tag, LV_ALIGN_TOP_LEFT, 0, 0);

    s_env_value_humi = lv_label_create(card_humi);
    lv_label_set_text(s_env_value_humi, "--.-");
    lv_obj_add_style(s_env_value_humi, &s_style_30, 0);
    lv_obj_set_style_text_color(s_env_value_humi, lv_color_hex(0x4FC3F7), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(s_env_value_humi, LV_ALIGN_LEFT_MID, 0, SCALE_DPX(14));

    s_env_unit_humi = lv_label_create(card_humi);
    lv_label_set_text(s_env_unit_humi, "%");
    lv_obj_add_style(s_env_unit_humi, &s_style_20, 0);
    lv_obj_set_style_text_color(s_env_unit_humi, lv_color_hex(0x81D4FA), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align_to(s_env_unit_humi, s_env_value_humi, LV_ALIGN_OUT_RIGHT_MID, SCALE_DPX(40), 0);

    s_env_hint = lv_label_create(s_env_panel);
    lv_label_set_text(s_env_hint, "Waiting for sensor data...");
    lv_obj_add_style(s_env_hint, &s_style_20, 0);
    lv_obj_set_width(s_env_hint, LV_PCT(100));
    lv_obj_set_style_text_align(s_env_hint, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_color(s_env_hint, lv_color_hex(0x93A4BD), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align_to(s_env_hint, card_row, LV_ALIGN_OUT_BOTTOM_MID, 0, SCALE_DPX(6));

    s_env_chart = lv_chart_create(s_env_panel);
    lv_obj_remove_flag(s_env_chart, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_size(s_env_chart, LV_PCT(100), SCALE_DPX(150));
    lv_obj_align_to(s_env_chart, btn_back, LV_ALIGN_OUT_TOP_MID, 0, SCALE_DPX(-10));
    lv_chart_set_type(s_env_chart, LV_CHART_TYPE_LINE);
    lv_chart_set_point_count(s_env_chart, ENV_CHART_POINTS);
    lv_chart_set_range(s_env_chart, LV_CHART_AXIS_PRIMARY_Y, -100, 600);
    lv_chart_set_range(s_env_chart, LV_CHART_AXIS_SECONDARY_Y, 0, 1000);
    lv_chart_set_div_line_count(s_env_chart, 5, 6);
    lv_obj_set_style_bg_opa(s_env_chart, LV_OPA_10, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(s_env_chart, lv_color_hex(0x0B1220), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(s_env_chart, 1, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_color(s_env_chart, lv_color_hex(0x22314A), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_radius(s_env_chart, SCALE_DPX(14), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(s_env_chart, SCALE_DPX(10), LV_PART_MAIN | LV_STATE_DEFAULT);

    s_env_ser_temp = lv_chart_add_series(s_env_chart, lv_color_hex(0xFFB74D), LV_CHART_AXIS_PRIMARY_Y);
    s_env_ser_humi = lv_chart_add_series(s_env_chart, lv_color_hex(0x4FC3F7), LV_CHART_AXIS_SECONDARY_Y);
    for (int i = 0; i < ENV_CHART_POINTS; i++)
    {
        s_env_temp_points[i] = LV_CHART_POINT_NONE;
        s_env_humi_points[i] = LV_CHART_POINT_NONE;
        lv_chart_set_value_by_id(s_env_chart, s_env_ser_temp, i, LV_CHART_POINT_NONE);
        lv_chart_set_value_by_id(s_env_chart, s_env_ser_humi, i, LV_CHART_POINT_NONE);
    }

    lv_obj_t *legend = lv_label_create(s_env_chart);
    lv_label_set_text(legend, "Temp (C)   Humi (%)");
    lv_obj_add_style(legend, &s_style_20, 0);
    lv_obj_set_style_text_color(legend, lv_color_hex(0x93A4BD), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_align(legend, LV_ALIGN_TOP_RIGHT, 0, SCALE_DPX(-6));
...
void xiaozhi_ui_show_meow_screen(void)
{
    /* 云端 meow_control(start) 同步：仅切到猫叫界面并置 UI 状态，
     * 检测线程与语音挂起已由云端命令处理方完成，这里不重复启动。 */
    if (!s_meow_screen)
    {
        meow_build_screen();
    }
...
void xiaozhi_ui_show_main_screen(void)
{
    /* 云端 meow_control(stop) 同步：仅回主界面，停止检测/恢复语音由命令处理方完成。 */
    s_meow_mode = RT_FALSE;
    if (s_main_screen)
        lv_screen_load(s_main_screen);
}
...
static void meow_back_btn_event_cb(lv_event_t *e)
{
    ...
    if (s_meow_mode)
    {
        ...
        xz_voice_resume();
        s_meow_mode = RT_FALSE;
        xz_notify_meow_state(RT_FALSE);   /* 板子退出猫叫页 -> 通知云端停止 */
    }
...
static void static_face_set_expression(int index)
{
    if (!s_face_left_eye || !s_face_right_eye || !s_face_mouth)
        return;

    const char *mouth = "___";
    ...
    lv_label_set_text(s_face_left_eye, index == 12 ? "-" : "o");
    lv_label_set_text(s_face_right_eye, "o");
    lv_label_set_text(s_face_mouth, mouth);
    if (s_face_label)
        lv_label_set_text(s_face_label, label);
}

static lv_obj_t *static_face_create(lv_obj_t *parent)
{
    lv_obj_t *face = lv_obj_create(parent);
    lv_obj_remove_flag(face, LV_OBJ_FLAG_SCROLLABLE);
    lv_obj_set_size(face, SCALE_DPX(360), SCALE_DPX(250));
    lv_obj_align(face, LV_ALIGN_CENTER, 0, SCALE_DPX(-30));
    lv_obj_set_style_bg_opa(face, LV_OPA_0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_border_width(face, 0, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_pad_all(face, 0, LV_PART_MAIN | LV_STATE_DEFAULT);

    s_face_left_eye = lv_label_create(face);
    lv_label_set_text(s_face_left_eye, "o");
    lv_obj_add_style(s_face_left_eye, &s_style_30, 0);
    lv_obj_set_style_text_color(s_face_left_eye, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(s_face_left_eye, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_width(s_face_left_eye, SCALE_DPX(80));
    lv_obj_align(s_face_left_eye, LV_ALIGN_CENTER, SCALE_DPX(-80), SCALE_DPX(-30));

    s_face_right_eye = lv_label_create(face);
    lv_label_set_text(s_face_right_eye, "o");
    lv_obj_add_style(s_face_right_eye, &s_style_30, 0);
    lv_obj_set_style_text_color(s_face_right_eye, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(s_face_right_eye, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_width(s_face_right_eye, SCALE_DPX(80));
    lv_obj_align(s_face_right_eye, LV_ALIGN_CENTER, SCALE_DPX(80), SCALE_DPX(-30));

    s_face_mouth = lv_label_create(face);
    lv_label_set_text(s_face_mouth, "___");
    lv_obj_add_style(s_face_mouth, &s_style_30, 0);
    lv_obj_set_style_text_color(s_face_mouth, lv_color_hex(0xFFFFFF), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(s_face_mouth, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_width(s_face_mouth, SCALE_DPX(180));
    lv_obj_align(s_face_mouth, LV_ALIGN_CENTER, 0, SCALE_DPX(55));

    s_face_label = lv_label_create(face);
    lv_label_set_text(s_face_label, "ready");
    lv_obj_add_style(s_face_label, &s_style_20, 0);
    lv_obj_set_style_text_color(s_face_label, lv_color_hex(0x888888), LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_style_text_align(s_face_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN | LV_STATE_DEFAULT);
    lv_obj_set_width(s_face_label, LV_PCT(100));
    lv_obj_align(s_face_label, LV_ALIGN_BOTTOM_MID, 0, 0);
...
static rt_err_t ui_objects_init(void)
{
    ...
    s_main_container = screen;
#if XIAOZHI_UI_USE_3D_EMOJI
    ...
    lv_obj_clear_flag(s_img_container, LV_OBJ_FLAG_CLICKABLE);

#if !XIAOZHI_UI_USE_3D_EMOJI
    s_emoji_fallback_canvas = lv_canvas_create(s_img_container);
    lv_obj_set_size(s_emoji_fallback_canvas, SCALE_DPX(420), SCALE_DPX(280));
    lv_obj_align(s_emoji_fallback_canvas, LV_ALIGN_TOP_MID, 0, SCALE_DPX(60));
    emoji_draw_2d_fallback(s_emoji_fallback_canvas, 0);
#endif
    ...
    lv_obj_move_foreground(s_btn_meow);

    s_lbl_meow = lv_label_create(s_btn_meow);
    lv_label_set_text(s_lbl_meow, "Meow detect");
    lv_obj_center(s_lbl_meow);

    /* Temp/Humi button (main screen) */
    s_btn_env = lv_button_create(screen);
    lv_obj_set_size(s_btn_env, SCALE_DPX(200), SCALE_DPX(56));
    lv_obj_align(s_btn_env, LV_ALIGN_CENTER, 0, SCALE_DPX(190));
    lv_obj_add_event_cb(s_btn_env, env_btn_event_cb, LV_EVENT_ALL, NULL);
    lv_obj_move_foreground(s_btn_env);

    s_lbl_env = lv_label_create(s_btn_env);
    lv_label_set_text(s_lbl_env, "Temp/Humi");
    lv_obj_center(s_lbl_env);
...
#else
    emoji_draw_2d_fallback(s_emoji_fallback_canvas, index);
#endif
}
...
static void ui_process_message(const ui_msg_t *msg)
{
    ...
    case UI_CMD_SET_MEOW_RESULT:
        if (s_meow_label_result)
        {
            ...
            if (det)
            {
                lv_label_set_text(s_meow_label_result, "MEOW DETECTED");
            ...
            {
                lv_label_set_text(s_meow_label_result, "NO MEOW");
    ...
    case UI_CMD_SET_ENV_RESULT:
    {
        float temp = 0.0f;
        float humi = 0.0f;
        sscanf(msg->data, "%f,%f", &temp, &humi);
        s_env_temp = temp;
        s_env_humi = humi;
        s_env_valid = RT_TRUE;
        if (s_env_mode)
...
void xiaozhi_ui_set_env_result(float temp, float humi)
{
    char tmp[48];
    rt_snprintf(tmp, sizeof(tmp), "%.2f,%.2f", temp, humi);
    ui_send_message(UI_CMD_SET_ENV_RESULT, tmp, "0.0,0.0");
}
