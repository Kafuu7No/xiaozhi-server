...
/**
 * @brief Update meow detect screen result text (thread-safe)
 * @param detected True if meow detected
 * @param score Confidence score (0~1)
 */
void xiaozhi_ui_set_meow_result(bool detected, float score);

/**
 * @brief Update temperature/humidity data (thread-safe)
 * @param temp Temperature in Celsius
 * @param humi Relative humidity in percent
 */
void xiaozhi_ui_set_env_result(float temp, float humi);

/**
 * @brief Enter meow detect screen and start detection (for voice command)
 *
 * 该接口等价于在主界面点击一次 `Meow detect` 按钮：
 * - 暂停小智语音流水线
 * - 切换到猫叫检测界面
 * - 启动连续猫叫检测线程
 */
...
/**
 * @brief Navigate to the meow-detect screen WITHOUT starting the detector.
 *
 * 供云端 meow_control(start) 同步调用：检测线程和语音挂起已由调用方处理，
 * 这里只切到猫叫界面并置位 UI 状态。线程安全（在消息线程调用，与
 * xiaozhi_ui_enter_meow_mode_from_voice 相同）。
 */
void xiaozhi_ui_show_meow_screen(void);

/**
 * @brief Navigate back to the main screen WITHOUT stopping the detector.
 *
 * 供云端 meow_control(stop) 同步调用：停止检测/恢复语音已由调用方处理。
 */
