    ...
    while (RT_TRUE)
    {
        ...
        if (received_event & XZ_WAKEWORD_EVENT_START)
        {
            ...
            while (wakeword_enabled)
            {
                ...
                {
                    static uint32_t s_infer_error_count = 0;
                    s_infer_error_count++;
                    if (s_infer_error_count <= 3 || (s_infer_error_count % 50) == 0)
                    {
                        LOG_E("Inference failed (%d), count=%u", err, (unsigned)s_infer_error_count);
                    }
